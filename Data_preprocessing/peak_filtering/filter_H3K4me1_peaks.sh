...
cat ../../H3K4me1_peaks/*.bed | sed -e 's/peak_call\///' -e 's/_H3K4me1.*//' | sort -k1,1 -k2,2n | 
...
cat $OUT_DIR/cts_to_filter.txt | awk '{if($6 >= 15 || $7 >= 15) print $1"\t"$2"\t"$3"\t"$4"\t"$5}' |

#remove blacklisted regions.
bedtools intersect -a - -b $blacklist -v -sorted > $OUT_DIR/H3K4me1_peaks_filtered.bed

#Final check!

cat $OUT_DIR/H3K4me1_peaks_filtered.bed | awk '{if($5 <= 14) print }'
