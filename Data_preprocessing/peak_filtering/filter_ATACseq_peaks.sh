...
cat ../../ATACseq_peaks/*.bed | sed -e 's/peak_call\///' -e 's/_peak.*//' | sort -k1,1 -k2,2n |
bedtools merge -i - -sorted -d -304 -c 4,4 -o distinct,count_distinct > $OUT_DIR/cts.bed
...
cat $OUT_DIR/cts_to_filter.txt | awk '{if($6 >= 36 || $7 >= 36) print $1"\t"$2"\t"$3"\t"$4"\t"$5}' |

#remove blacklisted regions.
bedtools intersect -a - -b $blacklist -v -sorted > $OUT_DIR/ATACseq_peaks_filtered.bed

#Final check!

cat $OUT_DIR/ATACseq_peaks_filtered.bed | awk '{if($5 <= 17) print }'
