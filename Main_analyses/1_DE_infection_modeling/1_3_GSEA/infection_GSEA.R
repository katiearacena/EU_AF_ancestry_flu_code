...
for (i in 1:length(datatypes)){
    ...
    system(paste0("mkdir -p ", out_dir, data_type_i, "/"))
    results_dir <- paste0(out_dir, data_type_i, "/")
    
    ## Set file and working directory (on local computer)
    file_name <- paste0("resultsALL.txt")
    working_dir <- paste0("Outputs/", folder,"/", data_type_i, "_results/")
    ...
    if(data_type_i =="RNAseq"){
        ...
        ## Load DE results table.
        resultsALL <- read.csv(paste0(working_dir, file_name))
  ...
  ## for peak datatypes
    ...
    if(data_type_i =="WGBS"){
        ...
        ranks <- results_with_genes[, c("symbol", "t.statistic")]
        ## Order t statistic in descending order (from + to - t statistic)
        ranks_t <- ranks[order(-(ranks$t.statistic)), ]
        ranks_for_gsea <- setNames(ranks_t$t.statistic, ranks_t$symbol)
    }
