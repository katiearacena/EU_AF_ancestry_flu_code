...
for(j in 2:length(datatypes)-1){
  ...
  if(data_type_i=="WGBS"){
   ...
   keepLoci.ex <- which(rowSums(BS.cov[, BSobj.fit.nolow$Condition == "Mock"] >= 4) > 17 &
  ...
  write.table(importances, paste0(OUTPUTS_dir,"/importances_",data_type_i,"_mock.txt"), quote = FALSE)
