...
for(j in 1:length(datatypes)){
  ...
  write.table(importances, paste0(OUTPUTS_dir,"/importances_",data_type_i,"_",part,".txt"), quote = FALSE)
  ...
  rownames(rela_impo_outs) <- rownames(medians)
...
write.table(rela_impo_outs, paste0(OUTPUTS_dir,"/rela_impo_outs_", part, ".txt"), sep = ",", quote = FALSE)
