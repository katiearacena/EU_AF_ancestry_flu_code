...
system(paste0("mkdir -p Outputs/",folder,"/predicted_pop_diffs_SNPs_STRs/same_model/", data, "/"))
out_dir <- paste0("Outputs/",folder,"/predicted_pop_diffs_SNPs_STRs/same_model/", data, "/")
...
##########################################################################################
## Get the best SNPs and STRs that are significantly associated with popDE genes and    ##
## filter the genes and variant from genotype and expression tables                     ##
## to reduce the test number                                                            ##
##########################################################################################

## Load the variants significantly associated with popDE genes
popDE_genes <- as.data.frame(fread(paste0("Outputs/2_ancestry_effects_modeling/PopDE_analysis/", data, "/results/popDE_", CONDITION, ".txt")))
rownames(popDE_genes) <- popDE_genes$V1; popDE_genes$V1 <- NULL

if (data=="WGBS"){
	popDE_genes_sig <- subset(popDE_genes, popDE_genes[,7] < .10)
	} else {
	popDE_genes_sig <- subset(popDE_genes, popDE_genes[,6] < .10)
	}


## get lists of the union/intersect of genes popDE SNPs and popDE STRs.
bestSNPs <- as.data.frame(fread(paste0("Outputs/3_QTL_mapping/SNP-QTL_mapping/",data,"/", condition,"/", data,"_", condition,"/results_best_SNPs_with_qval.txt")))
rownames(bestSNPs) <- bestSNPs$V1; bestSNPs$V1 <- NULL
bestSNPs <- arrange(bestSNPs, gene)

bestSTRs <- as.data.frame(fread(paste0("Outputs/3_QTL_mapping/STR-QTL_mapping/",data,"/", condition,"/", data,"_", condition,"/results_best_STRs_with_qval.txt")))
rownames(bestSTRs) <- bestSTRs$V1; bestSTRs$V1 <- NULL
bestSTRs <- arrange(bestSTRs, gene)

genes_with_both <- intersect(bestSNPs$gene, bestSTRs$gene)
gene_of_interests <- intersect(genes_with_both, row.names(popDE_genes_sig))

## extract the genotypes of the significant SNPs
bestSNPs_subset <- subset(bestSNPs, bestSNPs$gene %in% gene_of_interests)
## only keep snp and gene columns
bestSNPs_subset <- select(bestSNPs_subset, snps, gene)
...
bestSTRs_subset <- subset(bestSTRs, bestSTRs$gene %in% gene_of_interests)
## only keep str and gene columns
bestSTRs_subset <- select(bestSTRs_subset, snps, gene)
## subset genotypes to pertinent ones
...
EXPRESSION <- expression[rownames(expression) %in% gene_of_interests,]

gene_of_interests <- gene_of_interests[gene_of_interests %in% rownames(EXPRESSION)]

SNPGENOTYPES_SUBSET <- SNPgenotypes_subset[gene_of_interests,]
STRGENOTYPES_SUBSET <- STRgenotypes_subset[gene_of_interests,]
...
for(j in 1:length(gene_of_interests)){
  if(j%%10 == 0) print(j)
  gene <- gene_of_interests[j]
  ...
  #gene_model=lm(reformulate(termlabels = vars, intercept = FALSE, response = 'EXPRESSION[gene,]'), data=COVARIATES)
  ## for each gene, store each output separately
  results=c(gene, coef(gene_model))

  if(j == 1){
    outs <- results
  }else{
    outs <- rbind(outs, results)
  }

}

model_coef <- as.data.frame(outs)
row.names(model_coef) <- model_coef$V1
model_coef$V1 <- NULL

indx <- sapply(model_coef, is.factor)
model_coef[indx] <- lapply(model_coef[indx], function(x) as.numeric(as.character(x)))


#######################################################
## Calculate predicted using QTL effect and genotype ##
#######################################################


## multiply genotypes by effect sizes
## for SNPs: genotype (0,1, or 2) * coeficient
SNPpopValues <- sweep(SNPGENOTYPES_SUBSET, MARGIN = 1, model_coef$SNP, `*`)


# STR
## for STRs: genotype (varies) * coeficient
STRpopValues <- sweep(STRGENOTYPES_SUBSET, MARGIN = 1, model_coef$STR, `*`)

## create predicted exp matrix (SNPs)
SNPpredicted_expression_matrix <- SNPpopValues
colnames(SNPpredicted_expression_matrix) <- paste0(colnames(SNPpredicted_expression_matrix),"_",condition)


## create predicted exp matrix (STRs)
STRpredicted_expression_matrix <- STRpopValues
colnames(STRpredicted_expression_matrix) <- paste0(colnames(STRpredicted_expression_matrix),"_",condition)


## add up SNPs- and STRs-predicted exp matrix
predicted_expression_matrix <- SNPpredicted_expression_matrix +STRpredicted_expression_matrix
## write these as the predicted expression matrices
write.table(predicted_expression_matrix, paste0(out_dir,condition,"_predicted_expression.txt"), quote = FALSE)
...
metadata=metadata[which(metadata$PopDE_set==1),]
#ensure that the Sample_ID are the rownames of the cols dataframe
rownames(metadata)=paste0(metadata$Genotyping_ID, "_", metadata$Condition)
#ensure that the cols dataframe is ordered alphabetically (based on sample_id)
metadata=metadata[order(rownames(metadata)),]

cols <- metadata[rownames(metadata) %in% colnames(predicted_expression_matrix),]
...
## write outs
write.table(betas, paste0(out_dir,condition,"_predicted_PopDE_betas.txt"), quote = FALSE)
