...
# for more info on pathways go here: https://www.gsea-msigdb.org/gsea/msigdb/collections.jsp
...
## Retrieve human genes for immunologic signatures
#m_df = msigdbr(species = "Homo sapiens", category = "C7")
## Retrieve human genes for Gene sets derived from the GO Biological Process Ontology
...
system(paste0("mkdir -p Outputs/",folder,"/cisreg_popDE_GSEA/"))
out_dir <- paste0("Outputs/",folder,"/cisreg_popDE_GSEA/")

## Load POPDE NI results
for (i in 1:length(datatypes)){
  data_type_i <- datatypes[i]
  ## Print for reference
  print(data_type_i)
  ## Set file and working directory for each datatype

  system(paste0("mkdir -p ", out_dir, data_type_i, "/popDE_NI/"))
  results_dir <- paste0(out_dir, data_type_i, "/popDE_NI/")
  
  file_name <- paste0("/popDE_NI.qval.txt")
  working_dir <- paste0("Outputs/4_cisregression_modeling/cisregSNPsSTRs/", data_type_i)
  ...
  if(data_type_i =="RNAseq"){
    ...
    ## get gene names from ensembe id 
    gene_names <- read.table("Inputs/ref/ensemble_id_positions_with_gene_names.txt")
    gene_names <- dplyr::select(gene_names, "gene_name", "Gene_ID")
    ...
    resultsALL$peak_ids <- rownames(resultsALL)
    significant_hits <- resultsALL[resultsALL$qvals_NI < .10, ]
  ...
  if(data_type_i =="WGBS"){
    ...
    ## Order t statistic in descending order (from + to - statistic)
...
for (i in 1:length(datatypes)){
  ...
  file_name <- paste0("/popDE_FLU.qval.txt")
  working_dir <- paste0("Outputs/4_cisregression_modeling/cisregSNPsSTRs/", data_type_i)

  #system(paste0("mkdir -p ", working_dir, "/GSEA_PopDE_NI"))
  #setwd(working_dir)
