...
for(i in 1:length(datatypes)){
  ...
  sig_popDR_res <- popDR[popDR$qvals_FLU < .05, ]
...
write.table(GSVEA_SETS_FOR_PLOTTING, file= paste0(out_dir, "popDR_GSVEA_SETS_FOR_PLOTTING_FDR.05.txt"))

GSVEA_sig_testing_res <- do.call(rbind.data.frame, sig_testing)
write.table(GSVEA_sig_testing_res, file=paste0(out_dir,"popDR_GSVEA_SETS_sig_testing_results_FDR.05.txt"))
