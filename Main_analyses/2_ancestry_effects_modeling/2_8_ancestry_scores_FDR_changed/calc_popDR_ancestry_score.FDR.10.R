...
system(paste0("mkdir -p Outputs/",folder,"/ancestry_score_FDR_changed/"))
out_dir <- paste0("Outputs/",folder,"/ancestry_score_FDR_changed/")

## only include datatypes with sig popDE genes (exclude H3K27me3 and WGBS)
datatypes <- c("RNAseq") #,"ATACseq","H3K27ac","H3K4me1", "H3K4me3")
...
for(i in 1:length(datatypes)){
  ...
  print(data_type_i)
    ...
    ## Load FC matrices
  file_name <- paste0("FC_for_plotting.txt")
  working_dir <- paste0("Outputs/2_ancestry_effects_modeling/PopDR_analysis/", data_type_i,"/results/")
  
  FC[[i]] <- read.table(paste0(working_dir, file_name))
  ...
  ## merge
  res[[i]] <- full_join(FC[[i]], results_with_genes[[i]], by="feature_id")
...
for(i in 1:length(datatypes)){
  ...
  sig_popDR_res <- popDR[popDR$qvals_FLU < .10, ]
...
write.table(GSVEA_SETS_FOR_PLOTTING, file= paste0(out_dir, "popDR_GSVEA_SETS_FOR_PLOTTING_FDR.10.txt"))

GSVEA_sig_testing_res <- do.call(rbind.data.frame, sig_testing)
write.table(GSVEA_sig_testing_res, file=paste0(out_dir,"popDR_GSVEA_SETS_sig_testing_results_FDR.10.txt"))
