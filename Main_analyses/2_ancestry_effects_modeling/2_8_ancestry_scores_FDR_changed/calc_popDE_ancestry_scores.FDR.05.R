...
for(i in 1:length(datatypes)){
  ...
  sig_popDE_res <- popDE_res[popDE_res$qvals_NI < .05 | popDE_res$qvals_Flu < .05, ]
  sig_popDE_res <- sig_popDE_res$feature
  length(sig_popDE_res)
  
  sig_popDE_NI <- popDE_res[popDE_res$qvals_NI < .05, ]
  sig_popDE_NI <- sig_popDE_NI$feature
  length(sig_popDE_NI)
  
  sig_popDE_Flu <- popDE_res[popDE_res$qvals_Flu < .05, ]
  ...
  if (data_type_i=="RNAseq"){
    ...
    gene_values_popDE_sig[[i]] <- dplyr::select(gene_values_popDE_sig[[i]], !c(feature_id, chr, pos, stat, pvals, fdrs, qvals, NI_mean, Flu_mean, diff, peak_ids, gene_ids))
  } else {
  ...
  NI_gsva <- gsva(as.matrix(NI_X2), gs2, verbose=FALSE)
...
write.table(GSVEA_SETS_FOR_PLOTTING, file= paste0(out_dir, "GSVEA_SETS_FOR_PLOTTING_FDR.05.txt"))

GSVEA_sig_testing_res <- do.call(rbind.data.frame, sig_testing)
write.table(GSVEA_sig_testing_res, file=paste0(out_dir,"GSVEA_SETS_sig_testing_results_FDR.05.txt"))
