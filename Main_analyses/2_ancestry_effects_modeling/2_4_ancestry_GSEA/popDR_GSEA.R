...
for (i in 1:length(datatypes)){
    ...
    system(paste0("mkdir -p ", out_dir, data_type_i, "/popDR/"))
    results_dir <- paste0(out_dir, data_type_i, "/popDR/")
    
    ## Set file and working directory (on local computer)
    file_name <- paste0("results_FLU.txt")
    working_dir <- paste0("Outputs/", folder,"/PopDR_analysis/", data_type_i, "/results/")
