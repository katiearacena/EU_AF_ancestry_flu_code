...
reads = read.table(paste0("Outputs/", folder,"/batch_corrected_cts_matrices/eRNA_batch.age.corrected.txt"), header=TRUE, row.names=1)
...
write.table(results_FLU_BH_q, paste0(out_dir,"/results/popDE_FLU.txt"))
write.table(results_NI_BH_q, paste0(out_dir, "/results/popDE_NI.txt"))
## Write voomed read counts for plotting
write.table(v_BOTH$E, paste0(out_dir, "/results/voomed_counts_for_plotting.txt"))
...
m.sig <- get_significant_results(m.c, thresh=thresh, sig_fn=get_lfsr)
...
results_FLU_lfsr <- merge(results_FLU, lfsr_FLU, by=0)
rownames(results_FLU_lfsr) <- results_FLU_lfsr$Row.names
...
results_NI_lfsr <- merge(results_NI, lfsr_NI, by=0)
rownames(results_NI_lfsr) <- results_NI_lfsr$Row.names
...
# Write mash results
write.table(results_FLU_lfsr, paste0(out_dir, "/mash_results/popDE_FLU.txt"))
write.table(results_NI_lfsr, paste0(out_dir, "/mash_results/popDE_NI.txt"))