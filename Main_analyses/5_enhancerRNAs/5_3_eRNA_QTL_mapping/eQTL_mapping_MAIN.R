...
##Load metadata
metadata_whole = read.table(paste0("Inputs/metadata/RNAseq_metadata.txt"))
## Load age and batch corrected voom normalized read counts
reads_whole = read.table(paste0("Outputs/5_enhancerRNAs/batch_corrected_cts_matrices/eRNA_batch.age.corrected.txt"), header=TRUE, row.names=1)

## Load positions file
genepos = read.table(paste0("Inputs/QTL_mapping/eRNA_positions.txt"),header = TRUE, stringsAsFactors = FALSE)
## Load SNP positions
