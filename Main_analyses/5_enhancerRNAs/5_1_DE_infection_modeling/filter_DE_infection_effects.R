...
setwd('../../../')
system(paste0("mkdir -p Outputs/",folder,"/DE_infection_effects"))
system(paste0("mkdir -p Outputs/",folder,"/DE_infection_effects/permuted_p_values/"))
## Set output directoy
results_dir <- paste0("Outputs/", folder,"/DE_infection_effects")
...
#Load eRNA counts (row 1 as rownames)
reads = read.table(paste0("Inputs/counts_matrices/eRNA.paired.counts.txt"), header=TRUE, row.names=1)

## load RNAseq reads
RNAseq_reads = read.table(paste0("Inputs/counts_matrices/RNAseq_filtered.counts.txt"), header=TRUE, row.names=1)
## remove Mock, EU122 from RNAseq reads
...
length(reorder_names) == dim(RNAseq_reads)[2]

#hist(log2(NI_avg), breaks=100000)
#hist(Flu_avg, breaks=100000)
#hist(NI_avg, xlim=c(0, 20), breaks=100000)
#hist(Flu_avg, xlim=c(0, 20), breaks=100000)

####################################
## Voom plot of unfiltered counts ##
####################################

## unfiltered counts
dim(reads)[1]

## combine RNAseq reads and unfiltered eRNA reads
## make sure colnames are in same order and match
length(which(colnames(RNAseq_reads)!=colnames(reads)))
gene_unfiltered_eRNA_reads <- rbind(reads, RNAseq_reads)
dim(gene_unfiltered_eRNA_reads)

## DGEList simple takes reads and converts to a DGE object format which is needed in downstream functions.
dgef <- DGEList(counts = gene_unfiltered_eRNA_reads)
...
## Plot voomed data
pdf(paste0(results_dir,"/voom_plot_pre_filtering.pdf"))
v <- voom(dgef, design, plot = TRUE)
dev.off()
...
## counts requiring average of greater or equal to 10 reads in at least 1/2 of the samples in each condition
reads$NI_greater_10 <- Reduce(`+`, lapply(reads[grep("_NI", colnames(reads))], `>=`, 10))
reads$Flu_greater_10 <- Reduce(`+`, lapply(reads[grep("_Flu", colnames(reads))], `>=`, 10))

## 39 samples for RNAseq so 20 is at least 1/2
dim(reads)[1]
## get dim of filtered
dim(reads[reads$NI_greater_10 >= 20 | reads$Flu_greater_10 >= 20, ])[1]

filtered_reads <- reads[reads$NI_greater_10 >= 20 | reads$Flu_greater_10 >= 20, ]

## remove reads with greater counts
eRNA_reads <- filtered_reads[!grepl("_greater_", colnames(filtered_reads))]
write.table(eRNA_reads, paste0(results_dir, "/eRNA_filtered_reads.txt"))

#################################
## normalize with RNAseq reads ##
#################################

## combine RNAseq reads and filtered eRNA reads
## make sure colnames are in same order and match
length(which(colnames(RNAseq_reads)!=colnames(eRNA_reads)))
gene_eRNA_reads <- rbind(eRNA_reads, RNAseq_reads)
dim(gene_eRNA_reads)
...
## DGEList simple takes reads and converts to a DGE object format which is needed in downstream functions.
dge <- DGEList(counts = gene_eRNA_reads)
## CalcNormFactors does normalization by library size (default is TMM)
...
## Plot voomed data
pdf(paste0(results_dir,"/voom_plot_post_filtering.pdf"))
...
## extract only eRNAs
corrected_expression <- corrected_expression[grep("chr", rownames(corrected_expression)), ]
weights <- weights[grep("chr", rownames(weights)), ]
## make sure have same number of starting eRNAs as ending.
dim(corrected_expression)[1] == dim(eRNA_reads)[1]

## Write batch-corrected expression and weights
write.table(corrected_expression, paste0(results_dir,"/corrected_expression.txt"), quote = FALSE, sep = ",")
...
numGenes_total <- dim(resultsALL)[1]
