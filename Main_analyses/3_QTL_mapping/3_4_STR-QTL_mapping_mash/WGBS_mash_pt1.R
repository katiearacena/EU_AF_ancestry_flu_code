...
system(paste0("mkdir -p Outputs/",folder,"/STR-QTL_mash/", data, "/"))
out_dir <- paste0("Outputs/",folder,"/STR-QTL_mash/", data, "/")


## Load and create mash inputs
results_original_NI =read.table(paste0("Outputs/3_QTL_mapping/STR-QTL_mapping/",data,"/NI/", data, "_NI/raw_results/result_original.txt"),header=TRUE)
results_original_Flu =read.table(paste0("Outputs/3_QTL_mapping/STR-QTL_mapping/",data,"/Flu/", data, "_Flu/raw_results/result_original.txt"),header=TRUE)
...
#FLU_beta_SE_matrix_ALL <- na.omit(FLU_beta_SE_matrix_ALL)
#NI_beta_SE_matrix_ALL <- na.omit(NI_beta_SE_matrix_ALL)
...
colnames(all_conditions) <- c("strs_flu", "gene_flu", "statistic_flu", "pvalue_flu", "FDR_flu", "flu_beta", "flu_beta_ses",
                               "strs_NI", "gene_NI", "statistic_NI", "pvalue_NI", "FDR_NI", "NI_beta", "NI_beta_ses")
...
data_STRONG_p6 = mash_set_data(as.matrix(betas_strong[5000001:nrow(betas_strong), ]), as.matrix(betas_ses_strong[5000001:nrow(betas_strong), ]), V = Vhat, zero_Bhat_Shat_reset=2.22044604925031e-16)
