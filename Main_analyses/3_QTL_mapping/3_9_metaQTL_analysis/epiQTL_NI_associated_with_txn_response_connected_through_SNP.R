...
## load eQTL full NI results
RNAseqFlu_full <- as.data.frame(fread(paste0("Outputs/3_QTL_mapping/SNP-QTL_mapping/RNAseq/Flu/RNAseq_Flu/raw_results/result_original.txt")))
RNAseqFlu_full$V1 <- NULL
...
## connect the gene to the peak through most significant epiQTL snp
## subset RNAseq matrix eqtl output to only keep snps in epiQTL best file
RNAseqFlu_full_selected_snps <- RNAseqFlu_full[RNAseqFlu_full$snps %in% epi_NI_best$snps, ]

## sort this file based on snp and pvalue
RNAseqFlu_full_selected_snps_ORDERED <- RNAseqFlu_full_selected_snps[order(RNAseqFlu_full_selected_snps[, "snps"], RNAseqFlu_full_selected_snps[, "pvalue"]), ]
RNAseqFlu_full_selected_snps_ORDERED_nodups <-RNAseqFlu_full_selected_snps_ORDERED[!duplicated(RNAseqFlu_full_selected_snps_ORDERED$snps), ]

## now need to connect these snps with the peaks
peaks_with_genes <- left_join(epi_NI_best, RNAseqFlu_full_selected_snps_ORDERED_nodups, by="snps")
peaks_with_genes <- dplyr::select(peaks_with_genes, gene.x, gene.y)
colnames(peaks_with_genes) <- c("feature", "ensemble_id")
...
connect_peak_gene_df <- full_join(peaks_with_genes, RNAseq_gene_names, "ensemble_id")
...
get_meta_geno_QTL<- function(row, qqnorm_r, genotypes){
  ...
  if(is.na(row_info$ensemble_id)==TRUE){
    ...
    print("no FC info gene connected to this peak")
...
saveRDS(binned_geno_list, paste0(out_dir, data_i, "_QTL_NI_txn_response_peak_gene_connected_through_SNP.RDS"))
