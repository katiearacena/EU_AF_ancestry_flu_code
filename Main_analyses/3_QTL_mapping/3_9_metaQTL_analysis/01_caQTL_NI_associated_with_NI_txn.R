...
## RNAseq NI matrix
RNAseq_reads <- as.data.frame(fread(paste0("Outputs/2_ancestry_effects_modeling/batch_corrected_cts_matrices/RNAseq_batch.age.corrected.txt")))
rownames(RNAseq_reads) <- RNAseq_reads$V1; RNAseq_reads$V1 <- NULL
RNAseq_NI <- RNAseq_reads[ , grepl( "NI" , colnames(RNAseq_reads))]
colnames(RNAseq_NI) <- str_remove(colnames(RNAseq_NI), "_NI")
...
RNAseq_qqnorm_FC <- qqnorm_data(RNAseq_NI)
...
saveRDS(binned_geno_list, paste0(out_dir, data_i, "_QTL_NI_with_NI_txn_pt1.RDS"))