### To integrate QTL across datatypes
...
system(paste0("mkdir -p Outputs/",folder,"/SNP-QTL_integration/", condition, "/"))
out_dir <- paste0("Outputs/",folder,"/SNP-QTL_integration/", condition, "/")
...
## load in real results for QTL of interest but permuted results for all other datatypes.
for (i in 1:length(datatypes)){
  data_i <- datatypes[i]
  print(data_i)
  if(i==QTL_type) {
    full_res[[QTL_type]] <- as.data.frame(fread(paste0("Outputs/3_QTL_mapping/SNP-QTL_mapping/", data_i, "/", condition, "/", data_i, "_", condition, "/raw_results/result_original.txt")))
    rownames(full_res[[QTL_type]]) <- full_res[[QTL_type]]$V1; full_res[[QTL_type]]$V1 <- NULL
 } else {
   full_res[[i]] <- as.data.frame(fread(paste0("Outputs/3_QTL_mapping/SNP-QTL_mapping/", data_i, "/", condition, "/", data_i, "_", condition, "/raw_results/result_permuted_1.txt")))
...
## set function to search for and collect top pvalue for each snp
...
## and for each datatype in snps_res
  for (j in 1:length(snps_res)){
    ...
    ## get the top pvalue from this list.
    top_pval_for_datatype <- all_pvals_for_gene[order(all_pvals_for_gene[, 2]), ][1, ]


## add pvalue of best snp to dataframe
    data_df[j, ] <- c(as.character(top_pval_for_datatype$V1), top_pval_for_datatype$V2)

  }

## now save this dataframe for each eGene
  gene_name_i <- names(sig_QTL_gene_list)[i]

  if(i==1){
    colnames(data_df) <- c(paste0(gene_name_i, "_snp"), paste0(gene_name_i, "_pval"))
    gene_data_df <- data_df
  } else {
    colnames(data_df) <- c(paste0(gene_name_i, "_snp"), paste0(gene_name_i, "_pval"))
    gene_data_df <- cbind(gene_data_df, data_df)
  }
}

fwrite(gene_data_df, paste0(out_dir, datatypes[QTL_type], "_", condition, "_QTL.integ.PERM1.txt"))