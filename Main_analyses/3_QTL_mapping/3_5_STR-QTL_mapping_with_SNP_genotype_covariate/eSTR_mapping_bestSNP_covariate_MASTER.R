...
system(paste0("mkdir -p Outputs/",folder,"/STR-QTL_mapping_SNPregression/eSTRs_SNPregression", data, "/", condition, "/", res_dir, "/"))
out_dir <- paste0("Outputs/",folder,"/STR-QTL_mapping_SNPregression/eSNPs_SNPregression", data, "/", condition, "/", res_dir, "/")
...
# Import the best SNPs genotypes to build the covariate table for individual genes
bestSNPs = read.table(paste0("Outputs/",folder,"/STR-QTL_mapping_SNPregression/corresponding_best_SNPs", data, "/", condition, "/", res_dir, "/results_best_SNPs_with_qval.txt" ))

SNPgtypes=read.table(paste0("Inputs/QTL_mapping/SNP_genotypes.txt"),header = TRUE, stringsAsFactors = FALSE)
SNPgenotype = SNPgenotype[,which(colnames(SNPgenotype) %in% rownames(metadata))]
...
length(which(rownames(genotypes)!=strspos$snp))
...
# Import the best STRs to limit the gene-STR pairs to be tested ??????

bestSTRs = read.table(paste0("Outputs/3_QTL_mapping/STR-QTL_mapping/",data, "/", condition, "/", data, "_", condition, "/results_best_SNPs_with_qval.txt"))
bestSTRs = bestSTRs[which(bestSTRs$qvalues<0.1),]

for (gene in bestSTRs$gene){
    #######################################
    ...
    EXPRESSION = as.matrix(EXPRESSION[which(row.names(EXPRESSION)==gene),])
    bestSNP = bestSNPs[which(bestSNPs$gene==gene),]$snps
    bestSNP_genotype = SNPgenotype[which(row.names(SNPgenotype)==bestSNP),]
    if (nrow(bestSNP_genotype)==1){
      COVARIATES = rbind(covariates,bestSNP_genotype)
    }else{
      COVARIATES = covariates
    }

    ## Subset genepos and snpspos to include only the gene
    GENEPOS = subset(genepos, genepos$Gene_ID==gene)
    bestSTR = as.character(bestSTRs[which(bestSTRs$gene==gene),]$strs)
    SNPSPOS = strspos[which(strspos$str == bestSTR),]
    GENOTYPES = genotypes[which(row.names(genotypes) == SNPSPOS$str),]
